      ...
      m_total_access_latency(SubsecondTime::Zero())
{
   ...
   if (Sim()->getCfg()->getBool("perf_model/dram/queue_model/enabled")) {
      m_queue_model_read = QueueModel::create("dram-queue-read", core_id,
                                              Sim()->getCfg()->getString("perf_model/dram/queue_model/type"),
                                              m_nvm_bandwidth.getRoundedLatency(8 * cache_block_size)); // bytes to bits
      m_queue_model_write = QueueModel::create("dram-queue-write", core_id,
                                               Sim()->getCfg()->getString("perf_model/dram/queue_model/type"),
                                               m_nvm_bandwidth.getRoundedLatency(
                                                     8 * cache_block_size)); // bytes to bits
   }
