...
if do_stats:
  ...
  if through_time:
    ...
    nameids = dict([ ('%s.%s' % (objectname, metricname), nameid) for nameid, (objectname, metricname) in list(names.items()) if '%s.%s' % (objectname, metricname) in metrics ])
    prefixes = stats.get_snapshots()
    prefixes_len = max(list(map(len, prefixes)))
    ...
    with sniper_lib.OutputToLess():
      for metric, _metric in zip(metrics, through_time):
        ...
        for prefix in prefixes:
          ...
          v = [ v.get(i, 0) for i in range(max(list(v.keys()) or [0])+1) ]
          v = do_op(op, state, v)
