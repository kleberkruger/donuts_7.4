...
class MemoryTracker:
  def __init__(self, resultsdir = '.'):
    ...
    self.hitwhere_load_global = dict([ (k.split('-', 3)[3], sum(v)) for k, v in list(stats.items()) if k.startswith('L1-D.loads-where-') ])
    self.hitwhere_load_unknown = self.hitwhere_load_global.copy()
    self.hitwhere_store_global = dict([ (k.split('-', 3)[3], sum(v)) for k, v in list(stats.items()) if k.startswith('L1-D.stores-where-') ])
    ...
    self.evicts_global = sum([ sum(v) for k, v in list(stats.items()) if re.match('L%d.evict-.$' % llc_level, k) ])
  ...
  def write(self, obj):
    ...
    for stack, site in sites_sorted:
      ...
      evicts = {}
      for _stack, _site in list(self.sites.items()):
        for _siteid, _cnt in list(_site.evictedby.items()):
      ...
      evicts = {}
      for siteid, cnt in list(site.evictedby.items()):
    ...
    for hitwhere in self.hitwheres:
      if self.hitwhere_load_global[hitwhere] + self.hitwhere_store_global[hitwhere]:
        ...
        for stack, site in sorted(list(self.sites.items()), key = lambda k_v: k_v[1].hitwhereload.get(hitwhere, 0) + k_v[1].hitwherestore.get(hitwhere, 0), reverse = True):
